...
MetricInfoArtifact = namedtuple("MetricInfoArtifact", ["model_name", "model_object", "train_accuracy","test_accuracy", 
                                                       ...
                                                       "train_roc_auc", "test_roc_auc","model_accuracy","index_number"])
...
def evaluate_classification_model(model_list: List, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    MetricInfoArtifact = namedtuple("MetricInfoArtifact", ["model_name", "model_object", "train_accuracy","test_accuracy", "train_precision","test_precision","train_recall","test_recall","train_f1_score","test_f1_score",
                                                       "train_roc_auc", "test_roc_auc","model_accuracy","index_number"])
    ...
    try:
        ...
        for model in model_list:
            ...
            # Calculating accuracy, precision, recall, F1-score, and ROC AUC
            ...
            # Calculating the difference between test and train accuracies
            diff_test_train_acc = abs(test_accuracy - train_accuracy)

            # Logging metrics
            logging.info(f"{'>>'*30} Metrics {'<<'*30}")
            logging.info(f"Train Accuracy\tTest Accuracy\tModel Accuracy")
            logging.info(f"{train_accuracy:.4f}\t\t{test_accuracy:.4f}\t\t{model_accuracy:.4f}")

            logging.info(f"Train Precision\tTest Precision")
            logging.info(f"{train_precision:.4f}\t\t{test_precision:.4f}")

            logging.info(f"Train Recall\tTest Recall")
            logging.info(f"{train_recall:.4f}\t\t{test_recall:.4f}")

            logging.info(f"Train F1-Score\tTest F1-Score")
            logging.info(f"{train_f1_score:.4f}\t\t{test_f1_score:.4f}")

            logging.info(f"Train ROC AUC\tTest ROC AUC")
            logging.info(f"{train_roc_auc:.4f}\t\t{test_roc_auc:.4f}")

            logging.info(f"Diff Test-Train Accuracy: {diff_test_train_acc:.4f}")

            # Checking if the model meets acceptance criteria
        ...
        # If no acceptable model is found
...
def get_sample_model_config_yaml_file(export_dir: str):
    ...
    try:
        ...
        # Define the path to the export YAML file
        export_file_path = os.path.join(export_dir, "model.yaml")

        # Write the model configuration dictionary to the YAML file
        with open(export_file_path, 'w') as file:
            yaml.dump(model_config, file)

        # Return the path to the exported YAML file
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref: object, property_data: dict):
        try:
            # Check if the property_data parameter is a dictionary
            if not isinstance(property_data, dict):
                raise Exception("property_data parameter required to be a dictionary")

            # Iterate over each key-value pair in the property_data dictionary
            for key, value in property_data.items():
                # Construct a logging message to show the attribute assignment
                logging.info(f"Executing: {str(instance_ref)}.{key} = {value}")
                
                # Use the setattr function to set the attribute value of the instance
                setattr(instance_ref, key, value)
            
            # Return the modified instance_ref after updating the properties
            return instance_ref
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        """
        execute_grid_search_operation(): This function performs a parameter search operation using grid search
        and returns the best optimistic model with the best parameters.

        Parameters:
        - initialized_model: An InitializedModelDetail object containing the model to be evaluated and its parameter grid.
        - input_feature: Input features for training the model.
        - output_feature: Target/dependent features for training the model.

        Returns:
        - A GridSearchedBestModel object containing information about the best model found.
        """
        try:
            ...
            # Create a GridSearchedBestModel object containing the best model information
